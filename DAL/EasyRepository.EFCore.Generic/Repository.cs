...
namespace EasyRepository.EFCore.Generic
{
    ...
    internal sealed class Repository : IRepository
    {
        ...
        //public async Task<List<TEntity>> ExecuteStoredProcedure<TEntity>(string storedProcedureName)
        //{
        //    var result = await _context.Set<TEntity>().FromSqlRaw(storedProcedureName).ToListAsync();
        //    return result;
        //}
        //public async Task<List<TEntity>> ExecuteStoredProcedure<TEntity>(string storedProcedureName, params SqlParameterExpression[] parameters)
        //{
        //    var result = await Database.SqlQueryRaw<TEntity>(storedProcedureName, parameters).ToListAsync();
        //    return result;
        //}
        ...
        public async Task<List<TEntity>> ExecuteStoredProcedure<TEntity>(string sp) where TEntity : class
        {
            var result = await _context.Set<TEntity>().FromSqlRaw(sp).ToListAsync();
            return result;
        }
        public async Task<List<TEntity>> ExecuteStoredProcedure<TEntity>(string sp, params SqlParameterExpression[] parameters) where TEntity : class
        {
            var result = await _context.Set<TEntity>().FromSqlRaw(sp).ToListAsync();
            return result;
        }
        ...
        public async Task<List<object>> ExecuteStoredProcedureDynamic(string sp, params SqlParameter[] parameters)
        {
            var parameterNames = parameters.Select(p => p.ParameterName).ToArray();
            var parameterValues = parameters.Select(p => p.Value).ToArray();
            var query = $"{sp} {string.Join(", ", parameterNames)}";
                ...
                if (check != null)
                {
                    using (var reader = command.ExecuteReader())
                    {
                        ...
                        while (reader.Read())
                        {
                            ...
                            var dynamicDictionary = (IDictionary<string, object>)dynamicObject;
                            for (var i = 0; i < reader.FieldCount; i++)
                            {
                                ...
                                {
                                    columnValue = (columnValue is DateTime dateTimeValue) ? dateTimeValue.ToString("yyyy-MM-dd HH:mm:ss") : DBNull.Value;
                                }
                                else if (valueType == typeof(decimal))
                                {
                                    columnValue = (columnValue != null && !DBNull.Value.Equals(columnValue)) ? Convert.ToDecimal(columnValue) : 0;
                                }
                                else if (valueType == typeof(int))
                                {
                                    columnValue = (columnValue != null && !DBNull.Value.Equals(columnValue)) ? Convert.ToInt32(columnValue) : 0;
                                }
                                else
                                {
                                    columnValue = (columnValue != null && !DBNull.Value.Equals(columnValue)) ? columnValue.ToString() : string.Empty;
                                }
                                dynamicDictionary.Add(columnName, columnValue);
                            }
